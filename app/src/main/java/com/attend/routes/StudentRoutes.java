...
public class StudentRoutes {
    ...
    public void markAttendance(String rollno, String subject, String period, final ApiResponse<Models.MarkAttendanceOnServer> completion) {

        String url = Constants.URL_MARK_ATTENDANCE + rollno + "/" + subject + "/" + period;

        final Models.MarkAttendanceOnServer markAttendanceOnServer = new Models.MarkAttendanceOnServer();
        ...
        volleyHandler.RequestApi(url, new ApiResponse<ApiResult>() {
            ...
            public void onCompletion(ApiResult res) {
                if (res.success == 200 && res.dataIsObject()) {
                    JSONObject userObj = res.getDataAsObject();
                    Models.MarkAttendanceOnServer markAttendanceOnServer = new Gson().fromJson(userObj.toString(), Models.MarkAttendanceOnServer.class);
                    markAttendanceOnServer.message = res.message;
                    completion.onCompletion(markAttendanceOnServer);
                } else {
                    completion.onCompletion(markAttendanceOnServer);
                }
            }
